...
TabPanel.prototype = {
  ...
  init : function(){
    ...
    this.tabpanel_content.width(t_w);
	  this.tabpanel_content.height(t_h-this.tabpanel_tab_content.get(0).offsetHeight + 2);
  ...
  kill : function(position){
    ...
    //注册关闭时间
    if(tabEntity.tabs[position].beforeClose){
    	var result = tabEntity.tabs[position].beforeClose(position);
    	if(!result){
    		return;
    	}
    }
