...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			ts.p.cRecords = intNum($.jgrid.getAccessor(data,dReader.cRecords));
			...
			while (i<len) {
				...
				rd={};
				if(ts.p.showAll) {
					if(pre!=cur[ts.p.showAll]){
						pre=cur[ts.p.showAll];ir++;
					}
				} else {
					ir++;
				}
				i++;
				if(!ts.p.showAll && ir===rn) { break; }
			}
			if(ts.p.gridview === true ) {
				fpos = ts.p.treeANode > -1 ? ts.p.treeANode: 0;
				if(ts.p.grouping) {
			...
			ts.p.totaltime = new Date() - startReq;
			if(ir>0) {
				if(ts.p.showAll) {ts.p.records=ir}
				else if(ts.p.records===0) { ts.p.records=len; }
			}
			rowData = null;
			if( ts.p.treeGrid === true) {
				try {self.jqGrid("setTreeNode", fpos+1, ir+fpos+1);} catch (e) {}
